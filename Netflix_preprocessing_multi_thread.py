...
def query_tmdb(title):
    ...
    try:
        response = requests.get(TMDB_SEARCH_URL, params={"api_key":API_KEY, "query": title}).json()
        results = response.get("results")
        if not results:
            return None, None, None
        movie_id = results[0]['id']

        release_response = requests.get(TMDB_RELEASE_URL.format(id=movie_id), params={"api_key": API_KEY}).json()
        ...
        credits_data = requests.get(TMDB_CREDITS_URL.format(movie_id=movie_id), params={"api_key": API_KEY}).json()
...
def clean_netflix_data(df):
    ...
    df['Netflix Release Date'] = df['Netflix Release Date'].apply(convert_date_format)
